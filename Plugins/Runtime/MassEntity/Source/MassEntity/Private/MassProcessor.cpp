...
void UMassProcessor::CallExecute(FMassEntityManager& EntityManager, FMassExecutionContext& Context)
{
	...
	{
		const double StartTime = FPlatformTime::Seconds();	// @third party code - CleverCrowdSim Measuring execution time of all mass processors
		
		Execute(EntityManager, Context);
		
		const double EndTime = FPlatformTime::Seconds();
		EntityManager.TotalProcessorsExecutionTime += EndTime - StartTime;
	}
	else
	{
